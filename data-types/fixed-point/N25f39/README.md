...
An unsigned 64-bit "normed" fixed-point number with 39 fractional bits.
...
- **base_type**: `uint64`
- **f**: 39 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `3.3554432000061035e7`

The stored `UInt64` value `i` is interpreted as `i / (2^39 - 1)`. This is equivalent to `Normed{UInt64, 39}` in `FixedPointNumbers.jl`.
