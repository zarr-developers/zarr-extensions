...
A signed 64-bit fixed-point number with 36 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.34217728e8` to `1.34217728e8`

The stored `Int64` value `i` is interpreted as `i / 2^36`. This is equivalent to `Fixed{Int64, 36}` in `FixedPointNumbers.jl`.
