...
An unsigned 64-bit "normed" fixed-point number with 31 fractional bits.
...
- **base_type**: `uint64`
- **f**: 31 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8.589934596e9`

The stored `UInt64` value `i` is interpreted as `i / (2^31 - 1)`. This is equivalent to `Normed{UInt64, 31}` in `FixedPointNumbers.jl`.
