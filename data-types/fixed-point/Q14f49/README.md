...
A signed 64-bit fixed-point number with 49 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-16384.0` to `16384.0`

The stored `Int64` value `i` is interpreted as `i / 2^49`. This is equivalent to `Fixed{Int64, 49}` in `FixedPointNumbers.jl`.
