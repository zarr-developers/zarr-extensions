...
An unsigned 16-bit "normed" fixed-point number with 9 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `128.249`

The stored `UInt16` value `i` is interpreted as `i / (2^9 - 1)`. This is equivalent to `Normed{UInt16, 9}` in `FixedPointNumbers.jl`.
