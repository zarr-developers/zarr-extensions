...
An unsigned 64-bit "normed" fixed-point number with 35 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `5.36870912015625e8`

The stored `UInt64` value `i` is interpreted as `i / (2^35 - 1)`. This is equivalent to `Normed{UInt64, 35}` in `FixedPointNumbers.jl`.
