...
An unsigned 64-bit "normed" fixed-point number with 10 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.80320078921892e16`

The stored `UInt64` value `i` is interpreted as `i / (2^10 - 1)`. This is equivalent to `Normed{UInt64, 10}` in `FixedPointNumbers.jl`.
