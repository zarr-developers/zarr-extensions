...
A signed 16-bit fixed-point number with 5 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1024.0` to `1023.97`

The stored `Int16` value `i` is interpreted as `i / 2^5`. This is equivalent to `Fixed{Int16, 5}` in `FixedPointNumbers.jl`.
