...
A signed 32-bit fixed-point number with 28 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-8.0` to `7.999999996`

The stored `Int32` value `i` is interpreted as `i / 2^28`. This is equivalent to `Fixed{Int32, 28}` in `FixedPointNumbers.jl`.
