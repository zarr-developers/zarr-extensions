...
A signed 16-bit fixed-point number with 4 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2048.0` to `2047.94`

The stored `Int16` value `i` is interpreted as `i / 2^4`. This is equivalent to `Fixed{Int16, 4}` in `FixedPointNumbers.jl`.
