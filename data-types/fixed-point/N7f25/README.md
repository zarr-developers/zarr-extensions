...
An unsigned 32-bit "normed" fixed-point number with 25 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `128.00000378`

The stored `UInt32` value `i` is interpreted as `i / (2^25 - 1)`. This is equivalent to `Normed{UInt32, 25}` in `FixedPointNumbers.jl`.
