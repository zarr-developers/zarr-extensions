...
Defines an `array -> bytes` codec which makes zarr readers/writers compatible with [N5 blocks](https://github.com/saalfeldlab/n5).

## Codec name

The value of the `name` member in the codec object MUST be `n5`.
...
1. Parse the N5 header
2. Validate that the block mode and dimensionality are consistent with expectation
...
| block types | homogeneous, defined by array metadata | 'varlen' or 'object' mode, defined by chunk header |
