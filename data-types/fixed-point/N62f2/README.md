...
An unsigned 64-bit "normed" fixed-point number with 2 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `6.148914691236517e18`

The stored `UInt64` value `i` is interpreted as `i / (2^2 - 1)`. This is equivalent to `Normed{UInt64, 2}` in `FixedPointNumbers.jl`.
