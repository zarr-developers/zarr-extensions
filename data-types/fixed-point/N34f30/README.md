...
An unsigned 64-bit "normed" fixed-point number with 30 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.71798692e10`

The stored `UInt64` value `i` is interpreted as `i / (2^30 - 1)`. This is equivalent to `Normed{UInt64, 30}` in `FixedPointNumbers.jl`.
