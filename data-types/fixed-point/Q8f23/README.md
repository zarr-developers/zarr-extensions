...
A signed 32-bit fixed-point number with 23 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-256.0` to `255.9999999`

The stored `Int32` value `i` is interpreted as `i / 2^23`. This is equivalent to `Fixed{Int32, 23}` in `FixedPointNumbers.jl`.
