...
An unsigned 64-bit "normed" fixed-point number with 53 fractional bits.
...
The stored `UInt64` value `i` is interpreted as `i / (2^53 - 1)`. This is equivalent to `Normed{UInt64, 53}` in `FixedPointNumbers.jl`.
