...
An unsigned 64-bit "normed" fixed-point number with 22 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.39804755968025e12`

The stored `UInt64` value `i` is interpreted as `i / (2^22 - 1)`. This is equivalent to `Normed{UInt64, 22}` in `FixedPointNumbers.jl`.
