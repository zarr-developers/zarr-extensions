...
A signed 32-bit fixed-point number with 5 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-6.7108864e7` to `6.710886397e7`

The stored `Int32` value `i` is interpreted as `i / 2^5`. This is equivalent to `Fixed{Int32, 5}` in `FixedPointNumbers.jl`.
