...
An unsigned 32-bit "normed" fixed-point number with 16 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `65537.0`

The stored `UInt32` value `i` is interpreted as `i / (2^16 - 1)`. This is equivalent to `Normed{UInt32, 16}` in `FixedPointNumbers.jl`.
