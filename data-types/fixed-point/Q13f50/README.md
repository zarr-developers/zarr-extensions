...
A signed 64-bit fixed-point number with 50 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-8192.0` to `8192.0`

The stored `Int64` value `i` is interpreted as `i / 2^50`. This is equivalent to `Fixed{Int64, 50}` in `FixedPointNumbers.jl`.
