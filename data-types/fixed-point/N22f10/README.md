...
An unsigned 32-bit "normed" fixed-point number with 10 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.1984040029e6`

The stored `UInt32` value `i` is interpreted as `i / (2^10 - 1)`. This is equivalent to `Normed{UInt32, 10}` in `FixedPointNumbers.jl`.
