...
An unsigned 64-bit "normed" fixed-point number with 13 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.2520747251507205e15`

The stored `UInt64` value `i` is interpreted as `i / (2^13 - 1)`. This is equivalent to `Normed{UInt64, 13}` in `FixedPointNumbers.jl`.
