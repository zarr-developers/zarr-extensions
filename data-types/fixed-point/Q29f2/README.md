...
A signed 32-bit fixed-point number with 2 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-5.36870912e8` to `5.368709118e8`

The stored `Int32` value `i` is interpreted as `i / 2^2`. This is equivalent to `Fixed{Int32, 2}` in `FixedPointNumbers.jl`.
