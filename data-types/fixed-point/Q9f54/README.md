...
A signed 64-bit fixed-point number with 54 fractional bits.
...
- **base_type**: `int64`
- **f**: 54 (number of fractional bits)
- **scaling**: by `2^f`
- **range**: `-512.0` to `512.0`

The stored `Int64` value `i` is interpreted as `i / 2^54`. This is equivalent to `Fixed{Int64, 54}` in `FixedPointNumbers.jl`.
