...
An unsigned 16-bit "normed" fixed-point number with 6 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1040.24`

The stored `UInt16` value `i` is interpreted as `i / (2^6 - 1)`. This is equivalent to `Normed{UInt16, 6}` in `FixedPointNumbers.jl`.
