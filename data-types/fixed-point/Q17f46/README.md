...
A signed 64-bit fixed-point number with 46 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-131072.0` to `131072.0`

The stored `Int64` value `i` is interpreted as `i / 2^46`. This is equivalent to `Fixed{Int64, 46}` in `FixedPointNumbers.jl`.
