...
A signed 64-bit fixed-point number with 34 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-5.36870912e8` to `5.36870912e8`

The stored `Int64` value `i` is interpreted as `i / 2^34`. This is equivalent to `Fixed{Int64, 34}` in `FixedPointNumbers.jl`.
