...
An unsigned 32-bit "normed" fixed-point number with 13 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `524352.0077`

The stored `UInt32` value `i` is interpreted as `i / (2^13 - 1)`. This is equivalent to `Normed{UInt32, 13}` in `FixedPointNumbers.jl`.
