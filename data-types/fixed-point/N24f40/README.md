...
An unsigned 64-bit "normed" fixed-point number with 40 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.677721600001526e7`

The stored `UInt64` value `i` is interpreted as `i / (2^40 - 1)`. This is equivalent to `Normed{UInt64, 40}` in `FixedPointNumbers.jl`.
