...
A signed 32-bit fixed-point number with 16 fractional bits.
...
The stored `Int32` value `i` is interpreted as `i / 2^16`. This is equivalent to `Fixed{Int32, 16}` in `FixedPointNumbers.jl`.
