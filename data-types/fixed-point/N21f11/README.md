...
An unsigned 32-bit "normed" fixed-point number with 11 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.0981764998e6`

The stored `UInt32` value `i` is interpreted as `i / (2^11 - 1)`. This is equivalent to `Normed{UInt32, 11}` in `FixedPointNumbers.jl`.
