...
An unsigned 64-bit "normed" fixed-point number with 25 fractional bits.
...
- **base_type**: `uint64`
- **f**: 25 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `5.497558302720005e11`

The stored `UInt64` value `i` is interpreted as `i / (2^25 - 1)`. This is equivalent to `Normed{UInt64, 25}` in `FixedPointNumbers.jl`.
