...
An unsigned 64-bit "normed" fixed-point number with 17 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.4073856210534406e14`

The stored `UInt64` value `i` is interpreted as `i / (2^17 - 1)`. This is equivalent to `Normed{UInt64, 17}` in `FixedPointNumbers.jl`.
