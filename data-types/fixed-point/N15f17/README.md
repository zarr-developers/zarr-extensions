...
An unsigned 32-bit "normed" fixed-point number with 17 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `32768.249994`

The stored `UInt32` value `i` is interpreted as `i / (2^17 - 1)`. This is equivalent to `Normed{UInt32, 17}` in `FixedPointNumbers.jl`.
