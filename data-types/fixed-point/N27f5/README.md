...
An unsigned 32-bit "normed" fixed-point number with 5 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.385473321e8`

The stored `UInt32` value `i` is interpreted as `i / (2^5 - 1)`. This is equivalent to `Normed{UInt32, 5}` in `FixedPointNumbers.jl`.
