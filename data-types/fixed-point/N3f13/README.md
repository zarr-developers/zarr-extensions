...
An unsigned 16-bit "normed" fixed-point number with 13 fractional bits.
...
The stored `UInt16` value `i` is interpreted as `i / (2^13 - 1)`. This is equivalent to `Normed{UInt16, 13}` in `FixedPointNumbers.jl`.
