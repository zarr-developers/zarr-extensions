...
An unsigned 64-bit "normed" fixed-point number with 52 fractional bits.
...
- **base_type**: `uint64`
- **f**: 52 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4096.000000000001`

The stored `UInt64` value `i` is interpreted as `i / (2^52 - 1)`. This is equivalent to `Normed{UInt64, 52}` in `FixedPointNumbers.jl`.
