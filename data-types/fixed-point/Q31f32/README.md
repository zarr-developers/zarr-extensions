...
A signed 64-bit fixed-point number with 32 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2.147483648e9` to `2.147483648e9`

The stored `Int64` value `i` is interpreted as `i / 2^32`. This is equivalent to `Fixed{Int64, 32}` in `FixedPointNumbers.jl`.
