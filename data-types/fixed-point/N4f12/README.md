...
An unsigned 16-bit "normed" fixed-point number with 12 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `16.0037`

The stored `UInt16` value `i` is interpreted as `i / (2^12 - 1)`. This is equivalent to `Normed{UInt16, 12}` in `FixedPointNumbers.jl`.
