...
An unsigned 64-bit "normed" fixed-point number with 36 fractional bits.
...
- **base_type**: `uint64`
- **f**: 36 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.6843545600390625e8`

The stored `UInt64` value `i` is interpreted as `i / (2^36 - 1)`. This is equivalent to `Normed{UInt64, 36}` in `FixedPointNumbers.jl`.
