...
A signed 32-bit fixed-point number with 31 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.0` to `0.9999999995`

The stored `Int32` value `i` is interpreted as `i / 2^31`. This is equivalent to `Fixed{Int32, 31}` in `FixedPointNumbers.jl`.
