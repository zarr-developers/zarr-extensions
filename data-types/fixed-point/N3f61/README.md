...
An unsigned 64-bit "normed" fixed-point number with 61 fractional bits.
...
- **base_type**: `uint64`
- **f**: 61 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8.0`

The stored `UInt64` value `i` is interpreted as `i / (2^61 - 1)`. This is equivalent to `Normed{UInt64, 61}` in `FixedPointNumbers.jl`.
