...
A signed 32-bit fixed-point number with 3 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2.68435456e8` to `2.684354559e8`

The stored `Int32` value `i` is interpreted as `i / 2^3`. This is equivalent to `Fixed{Int32, 3}` in `FixedPointNumbers.jl`.
