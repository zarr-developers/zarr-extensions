...
An unsigned 64-bit "normed" fixed-point number with 3 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.6352491533870787e18`

The stored `UInt64` value `i` is interpreted as `i / (2^3 - 1)`. This is equivalent to `Normed{UInt64, 3}` in `FixedPointNumbers.jl`.
