...
An unsigned 64-bit "normed" fixed-point number with 23 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.1990235176960312e12`

The stored `UInt64` value `i` is interpreted as `i / (2^23 - 1)`. This is equivalent to `Normed{UInt64, 23}` in `FixedPointNumbers.jl`.
