...
An unsigned 32-bit "normed" fixed-point number with 24 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `256.0000152`

The stored `UInt32` value `i` is interpreted as `i / (2^24 - 1)`. This is equivalent to `Normed{UInt32, 24}` in `FixedPointNumbers.jl`.
