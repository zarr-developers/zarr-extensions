...
An unsigned 32-bit "normed" fixed-point number with 8 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.6843009e7`

The stored `UInt32` value `i` is interpreted as `i / (2^8 - 1)`. This is equivalent to `Normed{UInt32, 8}` in `FixedPointNumbers.jl`.
