...
An unsigned 64-bit "normed" fixed-point number with 4 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.2297829382473034e18`

The stored `UInt64` value `i` is interpreted as `i / (2^4 - 1)`. This is equivalent to `Normed{UInt64, 4}` in `FixedPointNumbers.jl`.
