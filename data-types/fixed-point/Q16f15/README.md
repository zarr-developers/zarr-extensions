...
A signed 32-bit fixed-point number with 15 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-65536.0` to `65535.99997`

The stored `Int32` value `i` is interpreted as `i / 2^15`. This is equivalent to `Fixed{Int32, 15}` in `FixedPointNumbers.jl`.
