...
An unsigned 64-bit "normed" fixed-point number with 6 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.928054614874532e17`

The stored `UInt64` value `i` is interpreted as `i / (2^6 - 1)`. This is equivalent to `Normed{UInt64, 6}` in `FixedPointNumbers.jl`.
