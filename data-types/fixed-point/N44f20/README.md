...
An unsigned 64-bit "normed" fixed-point number with 20 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.7592202821648e13`

The stored `UInt64` value `i` is interpreted as `i / (2^20 - 1)`. This is equivalent to `Normed{UInt64, 20}` in `FixedPointNumbers.jl`.
