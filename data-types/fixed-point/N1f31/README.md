...
An unsigned 32-bit "normed" fixed-point number with 31 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.0000000005`

The stored `UInt32` value `i` is interpreted as `i / (2^31 - 1)`. This is equivalent to `Normed{UInt32, 31}` in `FixedPointNumbers.jl`.
