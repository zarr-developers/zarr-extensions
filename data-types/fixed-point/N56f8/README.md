...
An unsigned 64-bit "normed" fixed-point number with 8 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `7.234017283807667e16`

The stored `UInt64` value `i` is interpreted as `i / (2^8 - 1)`. This is equivalent to `Normed{UInt64, 8}` in `FixedPointNumbers.jl`.
