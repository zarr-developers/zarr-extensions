...
An unsigned 64-bit "normed" fixed-point number with 50 fractional bits.
...
- **base_type**: `uint64`
- **f**: 50 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `16384.00000000001`

The stored `UInt64` value `i` is interpreted as `i / (2^50 - 1)`. This is equivalent to `Normed{UInt64, 50}` in `FixedPointNumbers.jl`.
