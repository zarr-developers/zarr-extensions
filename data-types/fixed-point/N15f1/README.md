...
An unsigned 16-bit "normed" fixed-point number with 1 fractional bit.
...
The stored `UInt16` value `i` is interpreted as `i / (2^1 - 1)`. This is equivalent to `Normed{UInt16, 1}` in `FixedPointNumbers.jl`.
