...
A signed 32-bit fixed-point number with 8 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-8.388608e6` to `8.388607996e6`

The stored `Int32` value `i` is interpreted as `i / 2^8`. This is equivalent to `Fixed{Int32, 8}` in `FixedPointNumbers.jl`.
