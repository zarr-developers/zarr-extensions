...
An unsigned 32-bit "normed" fixed-point number with 2 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.431655765e9`

The stored `UInt32` value `i` is interpreted as `i / (2^2 - 1)`. This is equivalent to `Normed{UInt32, 2}` in `FixedPointNumbers.jl`.
