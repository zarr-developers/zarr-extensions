...
A signed 64-bit fixed-point number with 43 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.048576e6` to `1.048576e6`

The stored `Int64` value `i` is interpreted as `i / 2^43`. This is equivalent to `Fixed{Int64, 43}` in `FixedPointNumbers.jl`.
