...
An unsigned 64-bit "normed" fixed-point number with 21 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8.796097216514e12`

The stored `UInt64` value `i` is interpreted as `i / (2^21 - 1)`. This is equivalent to `Normed{UInt64, 21}` in `FixedPointNumbers.jl`.
