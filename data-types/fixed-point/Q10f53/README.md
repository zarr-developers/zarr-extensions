...
A signed 64-bit fixed-point number with 53 fractional bits.
...
- **base_type**: `int64`
- **f**: 53 (number of fractional bits)
- **scaling**: by `2^f`
- **range**: `-1024.0` to `1024.0`

The stored `Int64` value `i` is interpreted as `i / 2^53`. This is equivalent to `Fixed{Int64, 53}` in `FixedPointNumbers.jl`.
