...
An unsigned 64-bit "normed" fixed-point number with 51 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8192.000000000004`

The stored `UInt64` value `i` is interpreted as `i / (2^51 - 1)`. This is equivalent to `Normed{UInt64, 51}` in `FixedPointNumbers.jl`.
