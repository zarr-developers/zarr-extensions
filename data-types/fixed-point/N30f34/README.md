...
An unsigned 64-bit "normed" fixed-point number with 34 fractional bits.
...
- **base_type**: `uint64`
- **f**: 34 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.0737418240625e9`

The stored `UInt64` value `i` is interpreted as `i / (2^34 - 1)`. This is equivalent to `Normed{UInt64, 34}` in `FixedPointNumbers.jl`.
