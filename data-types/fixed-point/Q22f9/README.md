...
A signed 32-bit fixed-point number with 9 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-4.194304e6` to `4.194303998e6`

The stored `Int32` value `i` is interpreted as `i / 2^9`. This is equivalent to `Fixed{Int32, 9}` in `FixedPointNumbers.jl`.
