...
An unsigned 64-bit "normed" fixed-point number with 24 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.099511693312004e12`

The stored `UInt64` value `i` is interpreted as `i / (2^24 - 1)`. This is equivalent to `Normed{UInt64, 24}` in `FixedPointNumbers.jl`.
