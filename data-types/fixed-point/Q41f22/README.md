...
A signed 64-bit fixed-point number with 22 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2.199023255552e12` to `2.199023255552e12`

The stored `Int64` value `i` is interpreted as `i / 2^22`. This is equivalent to `Fixed{Int64, 22}` in `FixedPointNumbers.jl`.
