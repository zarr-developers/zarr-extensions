...
An unsigned 16-bit "normed" fixed-point number with 14 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.00018`

The stored `UInt16` value `i` is interpreted as `i / (2^14 - 1)`. This is equivalent to `Normed{UInt16, 14}` in `FixedPointNumbers.jl`.
