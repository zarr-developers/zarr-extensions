...
An unsigned 32-bit "normed" fixed-point number with 6 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `6.817408405e7`

The stored `UInt32` value `i` is interpreted as `i / (2^6 - 1)`. This is equivalent to `Normed{UInt32, 6}` in `FixedPointNumbers.jl`.
