...
A signed 32-bit fixed-point number with 12 fractional bits.
...
The stored `Int32` value `i` is interpreted as `i / 2^12`. This is equivalent to `Fixed{Int32, 12}` in `FixedPointNumbers.jl`.
