...
An unsigned 32-bit "normed" fixed-point number with 15 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `131076.00009`

The stored `UInt32` value `i` is interpreted as `i / (2^15 - 1)`. This is equivalent to `Normed{UInt32, 15}` in `FixedPointNumbers.jl`.
