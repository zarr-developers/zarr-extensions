...
An unsigned 32-bit "normed" fixed-point number with 12 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.0488320623e6`

The stored `UInt32` value `i` is interpreted as `i / (2^12 - 1)`. This is equivalent to `Normed{UInt32, 12}` in `FixedPointNumbers.jl`.
