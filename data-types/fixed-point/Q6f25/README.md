...
A signed 32-bit fixed-point number with 25 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-64.0` to `63.99999997`

The stored `Int32` value `i` is interpreted as `i / 2^25`. This is equivalent to `Fixed{Int32, 25}` in `FixedPointNumbers.jl`.
