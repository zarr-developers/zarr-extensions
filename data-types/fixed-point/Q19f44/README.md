...
A signed 64-bit fixed-point number with 44 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-524288.0` to `524288.0`

The stored `Int64` value `i` is interpreted as `i / 2^44`. This is equivalent to `Fixed{Int64, 44}` in `FixedPointNumbers.jl`.
