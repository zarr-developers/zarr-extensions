...
An unsigned 32-bit "normed" fixed-point number with 30 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.0000000028`

The stored `UInt32` value `i` is interpreted as `i / (2^30 - 1)`. This is equivalent to `Normed{UInt32, 30}` in `FixedPointNumbers.jl`.
