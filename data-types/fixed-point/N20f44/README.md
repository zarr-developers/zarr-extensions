...
An unsigned 64-bit "normed" fixed-point number with 44 fractional bits.
...
- **base_type**: `uint64`
- **f**: 44 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1048576.00000006`

The stored `UInt64` value `i` is interpreted as `i / (2^44 - 1)`. This is equivalent to `Normed{UInt64, 44}` in `FixedPointNumbers.jl`.
