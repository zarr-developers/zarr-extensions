...
An unsigned 64-bit "normed" fixed-point number with 1 fractional bit.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.8446744073709552e19`

The stored `UInt64` value `i` is interpreted as `i / (2^1 - 1)`. This is equivalent to `Normed{UInt64, 1}` in `FixedPointNumbers.jl`.
