...
A signed 32-bit fixed-point number with 13 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-262144.0` to `262143.9999`

The stored `Int32` value `i` is interpreted as `i / 2^13`. This is equivalent to `Fixed{Int32, 13}` in `FixedPointNumbers.jl`.
