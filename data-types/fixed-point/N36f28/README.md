...
An unsigned 64-bit "normed" fixed-point number with 28 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `6.8719476992e10`

The stored `UInt64` value `i` is interpreted as `i / (2^28 - 1)`. This is equivalent to `Normed{UInt64, 28}` in `FixedPointNumbers.jl`.
