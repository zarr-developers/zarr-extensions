...
A signed 64-bit fixed-point number with 52 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2048.0` to `2048.0`

The stored `Int64` value `i` is interpreted as `i / 2^52`. This is equivalent to `Fixed{Int64, 52}` in `FixedPointNumbers.jl`.
