...
An unsigned 64-bit "normed" fixed-point number with 38 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `6.710886400024414e7`

The stored `UInt64` value `i` is interpreted as `i / (2^38 - 1)`. This is equivalent to `Normed{UInt64, 38}` in `FixedPointNumbers.jl`.
