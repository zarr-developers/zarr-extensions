...
An unsigned 64-bit "normed" fixed-point number with 56 fractional bits.
...
The stored `UInt64` value `i` is interpreted as `i / (2^56 - 1)`. This is equivalent to `Normed{UInt64, 56}` in `FixedPointNumbers.jl`.
