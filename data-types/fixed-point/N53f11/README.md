...
An unsigned 64-bit "normed" fixed-point number with 11 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `9.011599449784832e15`

The stored `UInt64` value `i` is interpreted as `i / (2^11 - 1)`. This is equivalent to `Normed{UInt64, 11}` in `FixedPointNumbers.jl`.
