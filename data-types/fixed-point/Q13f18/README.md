...
A signed 32-bit fixed-point number with 18 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-8192.0` to `8191.999996`

The stored `Int32` value `i` is interpreted as `i / 2^18`. This is equivalent to `Fixed{Int32, 18}` in `FixedPointNumbers.jl`.
