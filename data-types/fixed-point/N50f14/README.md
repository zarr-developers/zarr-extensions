...
An unsigned 64-bit "normed" fixed-point number with 14 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.12596863051392e15`

The stored `UInt64` value `i` is interpreted as `i / (2^14 - 1)`. This is equivalent to `Normed{UInt64, 14}` in `FixedPointNumbers.jl`.
