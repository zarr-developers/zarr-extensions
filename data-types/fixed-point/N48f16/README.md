...
An unsigned 64-bit "normed" fixed-point number with 16 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.81479271743489e14`

The stored `UInt64` value `i` is interpreted as `i / (2^16 - 1)`. This is equivalent to `Normed{UInt64, 16}` in `FixedPointNumbers.jl`.
