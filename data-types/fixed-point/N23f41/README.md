...
An unsigned 64-bit "normed" fixed-point number with 41 fractional bits.
...
- **base_type**: `uint64`
- **f**: 41 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8388608.000003815`

The stored `UInt64` value `i` is interpreted as `i / (2^41 - 1)`. This is equivalent to `Normed{UInt64, 41}` in `FixedPointNumbers.jl`.
