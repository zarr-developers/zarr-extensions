...
An unsigned 16-bit "normed" fixed-point number with 5 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2114.03`

The stored `UInt16` value `i` is interpreted as `i / (2^5 - 1)`. This is equivalent to `Normed{UInt16, 5}` in `FixedPointNumbers.jl`.
