...
A signed 32-bit fixed-point number with 7 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.6777216e7` to `1.6777215992e7`

The stored `Int32` value `i` is interpreted as `i / 2^7`. This is equivalent to `Fixed{Int32, 7}` in `FixedPointNumbers.jl`.
