...
An unsigned 64-bit "normed" fixed-point number with 26 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.7487791104000006e11`

The stored `UInt64` value `i` is interpreted as `i / (2^26 - 1)`. This is equivalent to `Normed{UInt64, 26}` in `FixedPointNumbers.jl`.
