...
An unsigned 32-bit "normed" fixed-point number with 19 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8192.015623`

The stored `UInt32` value `i` is interpreted as `i / (2^19 - 1)`. This is equivalent to `Normed{UInt32, 19}` in `FixedPointNumbers.jl`.
