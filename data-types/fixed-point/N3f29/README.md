...
An unsigned 32-bit "normed" fixed-point number with 29 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `8.000000013`

The stored `UInt32` value `i` is interpreted as `i / (2^29 - 1)`. This is equivalent to `Normed{UInt32, 29}` in `FixedPointNumbers.jl`.
