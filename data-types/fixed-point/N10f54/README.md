...
An unsigned 64-bit "normed" fixed-point number with 54 fractional bits.
...
The stored `UInt64` value `i` is interpreted as `i / (2^54 - 1)`. This is equivalent to `Normed{UInt64, 54}` in `FixedPointNumbers.jl`.
