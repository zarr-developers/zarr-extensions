...
An unsigned 64-bit "normed" fixed-point number with 15 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `5.629671338148e14`

The stored `UInt64` value `i` is interpreted as `i / (2^15 - 1)`. This is equivalent to `Normed{UInt64, 15}` in `FixedPointNumbers.jl`.
