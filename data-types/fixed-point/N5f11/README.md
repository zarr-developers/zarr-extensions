...
An unsigned 16-bit "normed" fixed-point number with 11 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `32.0151`

The stored `UInt16` value `i` is interpreted as `i / (2^11 - 1)`. This is equivalent to `Normed{UInt16, 11}` in `FixedPointNumbers.jl`.
