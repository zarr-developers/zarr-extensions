...
A signed 64-bit fixed-point number with 61 fractional bits.
...
The stored `Int64` value `i` is interpreted as `i / 2^61`. This is equivalent to `Fixed{Int64, 61}` in `FixedPointNumbers.jl`.
