...
A signed 32-bit fixed-point number with 14 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-131072.0` to `131071.99994`

The stored `Int32` value `i` is interpreted as `i / 2^14`. This is equivalent to `Fixed{Int32, 14}` in `FixedPointNumbers.jl`.
