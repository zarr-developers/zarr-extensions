...
A signed 16-bit fixed-point number with 2 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-8192.0` to `8191.8`

The stored `Int16` value `i` is interpreted as `i / 2^2`. This is equivalent to `Fixed{Int16, 2}` in `FixedPointNumbers.jl`.
