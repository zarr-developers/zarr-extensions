...
A signed 64-bit fixed-point number with 33 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.073741824e9` to `1.073741824e9`

The stored `Int64` value `i` is interpreted as `i / 2^33`. This is equivalent to `Fixed{Int64, 33}` in `FixedPointNumbers.jl`.
