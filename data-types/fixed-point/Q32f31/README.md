...
A signed 64-bit fixed-point number with 31 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-4.294967296e9` to `4.294967296e9`

The stored `Int64` value `i` is interpreted as `i / 2^31`. This is equivalent to `Fixed{Int64, 31}` in `FixedPointNumbers.jl`.
