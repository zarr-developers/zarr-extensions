...
An unsigned 64-bit "normed" fixed-point number with 18 fractional bits.
...
- **base_type**: `uint64`
- **f**: 18 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `7.0369012614144e13`

The stored `UInt64` value `i` is interpreted as `i / (2^18 - 1)`. This is equivalent to `Normed{UInt64, 18}` in `FixedPointNumbers.jl`.
