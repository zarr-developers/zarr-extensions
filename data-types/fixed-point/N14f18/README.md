...
An unsigned 32-bit "normed" fixed-point number with 18 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `16384.062496`

The stored `UInt32` value `i` is interpreted as `i / (2^18 - 1)`. This is equivalent to `Normed{UInt32, 18}` in `FixedPointNumbers.jl`.
