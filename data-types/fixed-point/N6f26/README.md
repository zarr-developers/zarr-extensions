...
An unsigned 32-bit "normed" fixed-point number with 26 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `64.00000094`

The stored `UInt32` value `i` is interpreted as `i / (2^26 - 1)`. This is equivalent to `Normed{UInt32, 26}` in `FixedPointNumbers.jl`.
