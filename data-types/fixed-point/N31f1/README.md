...
An unsigned 32-bit "normed" fixed-point number with 1 fractional bit.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.294967295e9`

The stored `UInt32` value `i` is interpreted as `i / (2^1 - 1)`. This is equivalent to `Normed{UInt32, 1}` in `FixedPointNumbers.jl`.
