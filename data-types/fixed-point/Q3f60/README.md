...
A signed 64-bit fixed-point number with 60 fractional bits.
...
The stored `Int64` value `i` is interpreted as `i / 2^60`. This is equivalent to `Fixed{Int64, 60}` in `FixedPointNumbers.jl`.
