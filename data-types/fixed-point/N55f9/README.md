...
An unsigned 64-bit "normed" fixed-point number with 9 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `3.609930347105587e16`

The stored `UInt64` value `i` is interpreted as `i / (2^9 - 1)`. This is equivalent to `Normed{UInt64, 9}` in `FixedPointNumbers.jl`.
