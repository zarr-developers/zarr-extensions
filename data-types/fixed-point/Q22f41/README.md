...
A signed 64-bit fixed-point number with 41 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-4.194304e6` to `4.194304e6`

The stored `Int64` value `i` is interpreted as `i / 2^41`. This is equivalent to `Fixed{Int64, 41}` in `FixedPointNumbers.jl`.
