...
An unsigned 64-bit "normed" fixed-point number with 55 fractional bits.
...
The stored `UInt64` value `i` is interpreted as `i / (2^55 - 1)`. This is equivalent to `Normed{UInt64, 55}` in `FixedPointNumbers.jl`.
