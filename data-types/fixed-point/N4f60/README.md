...
An unsigned 64-bit "normed" fixed-point number with 60 fractional bits.
...
- **base_type**: `uint64`
- **f**: 60 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `16.0`

The stored `UInt64` value `i` is interpreted as `i / (2^60 - 1)`. This is equivalent to `Normed{UInt64, 60}` in `FixedPointNumbers.jl`.
