...
A signed 64-bit fixed-point number with 56 fractional bits.
...
- **base_type**: `int64`
- **f**: 56 (number of fractional bits)
- **scaling**: by `2^f`
- **range**: `-128.0` to `128.0`

The stored `Int64` value `i` is interpreted as `i / 2^56`. This is equivalent to `Fixed{Int64, 56}` in `FixedPointNumbers.jl`.
