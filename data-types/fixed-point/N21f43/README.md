...
An unsigned 64-bit "normed" fixed-point number with 43 fractional bits.
...
- **base_type**: `uint64`
- **f**: 43 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2097152.000000238`

The stored `UInt64` value `i` is interpreted as `i / (2^43 - 1)`. This is equivalent to `Normed{UInt64, 43}` in `FixedPointNumbers.jl`.
