...
An unsigned 16-bit "normed" fixed-point number with 4 fractional bits.
...
The stored `UInt16` value `i` is interpreted as `i / (2^4 - 1)`. This is equivalent to `Normed{UInt16, 4}` in `FixedPointNumbers.jl`.
