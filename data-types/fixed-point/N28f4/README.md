...
An unsigned 32-bit "normed" fixed-point number with 4 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `2.86331153e8`

The stored `UInt32` value `i` is interpreted as `i / (2^4 - 1)`. This is equivalent to `Normed{UInt32, 4}` in `FixedPointNumbers.jl`.
