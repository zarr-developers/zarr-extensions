...
An unsigned 64-bit "normed" fixed-point number with 32 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `4.294967297e9`

The stored `UInt64` value `i` is interpreted as `i / (2^32 - 1)`. This is equivalent to `Normed{UInt64, 32}` in `FixedPointNumbers.jl`.
