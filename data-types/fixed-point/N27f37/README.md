...
An unsigned 64-bit "normed" fixed-point number with 37 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `1.3421772800097656e8`

The stored `UInt64` value `i` is interpreted as `i / (2^37 - 1)`. This is equivalent to `Normed{UInt64, 37}` in `FixedPointNumbers.jl`.
