...
An unsigned 64-bit "normed" fixed-point number with 59 fractional bits.
...
- **base_type**: `uint64`
- **f**: 59 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `32.0`

The stored `UInt64` value `i` is interpreted as `i / (2^59 - 1)`. This is equivalent to `Normed{UInt64, 59}` in `FixedPointNumbers.jl`.
