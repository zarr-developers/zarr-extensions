...
An unsigned 16-bit "normed" fixed-point number with 10 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `64.0616`

The stored `UInt16` value `i` is interpreted as `i / (2^10 - 1)`. This is equivalent to `Normed{UInt16, 10}` in `FixedPointNumbers.jl`.
