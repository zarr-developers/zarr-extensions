...
An unsigned 32-bit "normed" fixed-point number with 23 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `512.0000609`

The stored `UInt32` value `i` is interpreted as `i / (2^23 - 1)`. This is equivalent to `Normed{UInt32, 23}` in `FixedPointNumbers.jl`.
