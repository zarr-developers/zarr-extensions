...
An unsigned 16-bit "normed" fixed-point number with 8 fractional bits.
...
The stored `UInt16` value `i` is interpreted as `i / (2^8 - 1)`. This is equivalent to `Normed{UInt16, 8}` in `FixedPointNumbers.jl`.
