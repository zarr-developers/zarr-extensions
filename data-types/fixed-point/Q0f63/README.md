...
A signed 64-bit fixed-point number with 63 fractional bits.
...
The stored `Int64` value `i` is interpreted as `i / 2^63`. This is equivalent to `Fixed{Int64, 63}` in `FixedPointNumbers.jl`.
