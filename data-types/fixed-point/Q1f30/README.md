...
A signed 32-bit fixed-point number with 30 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-2.0` to `1.9999999991`

The stored `Int32` value `i` is interpreted as `i / 2^30`. This is equivalent to `Fixed{Int32, 30}` in `FixedPointNumbers.jl`.
