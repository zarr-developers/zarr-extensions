...
An unsigned 32-bit "normed" fixed-point number with 14 fractional bits.
...
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `262160.00092`

The stored `UInt32` value `i` is interpreted as `i / (2^14 - 1)`. This is equivalent to `Normed{UInt32, 14}` in `FixedPointNumbers.jl`.
