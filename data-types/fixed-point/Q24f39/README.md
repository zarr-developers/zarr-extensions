...
A signed 64-bit fixed-point number with 39 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-1.6777216e7` to `1.6777216e7`

The stored `Int64` value `i` is interpreted as `i / 2^39`. This is equivalent to `Fixed{Int64, 39}` in `FixedPointNumbers.jl`.
