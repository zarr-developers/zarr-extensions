...
An unsigned 64-bit "normed" fixed-point number with 46 fractional bits.
...
- **base_type**: `uint64`
- **f**: 46 (number of fractional bits)
- **scaling**: by `2^f - 1`
- **range**: `0.0` to `262144.0000000037`

The stored `UInt64` value `i` is interpreted as `i / (2^46 - 1)`. This is equivalent to `Normed{UInt64, 46}` in `FixedPointNumbers.jl`.
