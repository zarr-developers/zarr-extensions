...
A signed 16-bit fixed-point number with 11 fractional bits.
...
The stored `Int16` value `i` is interpreted as `i / 2^11`. This is equivalent to `Fixed{Int16, 11}` in `FixedPointNumbers.jl`.
