...
A signed 64-bit fixed-point number with 45 fractional bits.
...
- **scaling**: by `2^f`
- **range**: `-262144.0` to `262144.0`

The stored `Int64` value `i` is interpreted as `i / 2^45`. This is equivalent to `Fixed{Int64, 45}` in `FixedPointNumbers.jl`.
