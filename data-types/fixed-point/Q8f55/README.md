...
A signed 64-bit fixed-point number with 55 fractional bits.
...
- **base_type**: `int64`
- **f**: 55 (number of fractional bits)
- **scaling**: by `2^f`
- **range**: `-256.0` to `256.0`

The stored `Int64` value `i` is interpreted as `i / 2^55`. This is equivalent to `Fixed{Int64, 55}` in `FixedPointNumbers.jl`.
